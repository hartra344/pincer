...
- a live gateway log that keeps growing.
...
- **Gateway Logs:** open **Organize → Gateway Logs…** (or ⌘K → **Gateway Logs…**) to watch the simulated gateway's log grow. Send a message or trigger an approval and it shows up there. Try the level buttons, search, **Pause** and **Export…**.
...
- **Limited administration.** The demo simulates command policy, pairing requests and gateway logs, not general Gateway Settings, automations or plugins. For broader administration workflows, use the [mock gateway](../../development/mock-gateway/).
