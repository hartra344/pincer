...
func runDemo() async {
    ...
    // The demo starts with one approval pending in Forge's Main chat; it stays unanswered here.
    check(gateway.approvals.map(\.sessionKey) == ["agent:coder:main"]
          && gateway.approvals.first?.command == "git push --force origin main" && gateway.approvals.first?.expiresAt == nil,
          "demo starts with the seeded approval (\(gateway.approvals.map(\.command)))")
    await chat.send("please approve this")
    let approvalSeen = await waitFor("approval") { gateway.approvals.contains { $0.sessionKey == key } }
    check(approvalSeen, "demo approval surfaced")
    if let approval = gateway.approvals.first(where: { $0.sessionKey == key }) {
        ...
        check(!gateway.approvals.contains { $0.sessionKey == key } && gateway.approvals.count == 1, "demo approval resolved")
...
func runMenuBarDemo() async {
    ...
    check(inbox.needsYou.count == 1 && seeded?.target == Notifier.Target(gatewayId: gateway.id, sessionKey: coderKey)
          && seeded?.title == "Approve: git push --force origin main — 🛠️ Main · Forge",
          "the seeded approval needs you, in Forge's Main chat (\(inbox.needsYou.map(\.title)))")
    check(Set(inbox.unread.map(\.target.sessionKey)) == [homeLab, papers] && inbox.unreadCount == 2
          && inbox.unread.map(\.title) == ["🦞 home-lab · Claw", "🔭 Paper digest · Scout"],
          "unread lists home-lab and Paper digest (\(inbox.unread.map(\.title)))")
    check(inbox.running.isEmpty && !inbox.isCaughtUp && inbox.badgeText == "3", "nothing running; the icon shows 3")
    ...
    inbox = MenuBarInbox(app: app)
    check(markedRead && inbox.unreadCount == 1 && inbox.unread.map(\.target.sessionKey) == [homeLab], "a chat marked read leaves Unread")
    ...
    if let pending = gateway.approvals.first(where: { $0.sessionKey == coderKey }) {
        check(pending.id == "approval_seeded_push" && pending.cwd == "/home/claw/project" && pending.agentId == "coder"
              && pending.allowedDecisions == ["allow-once", "allow-always", "deny"] && pending.expiresAt == nil
              && !pending.isExpired(at: Date().addingTimeInterval(7 * 86_400)),
              "the seeded approval has its command details and never expires")
        let history = gateway.approvalHistory
        await history.refresh()
