...
- **Usage & Cost** (Gateway Settings → Usage, ⌘K → "Usage & Cost…") shows what the Gateway has spent on tokens: summary tiles, a daily chart split into input, output and cache tokens, a breakdown by model, provider or agent, the top sessions, and provider rate limits with reset times (`usage.status`, `usage.cost`, `sessions.usage`). Pick Today, 7 Days, 30 Days or a custom range. Select a session, or choose **Session Usage…** from a chat's ⋯ menu or ⌘K, for its totals, usage over time and message log (`sessions.usage.timeseries`, `sessions.usage.logs`). Costs are the Gateway's USD estimates and can be partial or unknown when a model has no pricing. Each part says so when the Gateway doesn't support it or the token lacks the scope.
